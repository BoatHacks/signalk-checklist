...
class ChecklistStore {
  ...
  async create ({ name }) {
    ...
    const id = await this._freshId(name)
    const list = {
      id,
  ...
  /** Finds an unused id for `name`, appending -2, -3, … on collision. */
  async _freshId (name) {
    let id = slugify(name)
    let suffix = 1
  ...
  /**
   * Copies a list's structure (name, items, retention setting) under a new
   * id, as a template for a new list — not its run-state: every item comes
   * back unchecked with any recorded value cleared, same as a brand new
   * list would start.
   */
  async duplicate (id) {
    ...
    const newId = await this._freshId(name)
    const list = {
      id: newId,
      name,
      items: existing.items.map((item) => ({
        ...item,
        id: newItemId(),
        ...(item.type === 'item' ? { checked: false, value: null } : {})
      })),
      retentionDays: existing.retentionDays,
      updatedAt: new Date().toISOString()
    }
    await atomicWriteJSON(this._fileFor(newId), list)
    return list
  }

  /**
   * Replace a list's structure (name + items + retention setting +
   * collapse-checked display setting). Last-write-wins: no locking, no
   * merge — whatever is saved last simply overwrites what was there.
   */
  async saveStructure (id, { name, items, retentionDays, collapseChecked }) {
    const existing = await this.get(id)
    if (!existing) throw new Error(`list not found: ${id}`)
    ...
    const updated = {
      ...
      collapseChecked: collapseChecked !== undefined
        ? Boolean(collapseChecked)
        : Boolean(existing.collapseChecked),
      updatedAt: new Date().toISOString()
    }
  ...
  async importList (doc) {
    ...
    const list = {
      ...
      collapseChecked: Boolean(doc.collapseChecked),
      updatedAt: new Date().toISOString()
    }
    await atomicWriteJSON(this._fileFor(id), list)
    return list
...
function normalizeItem (item) {
  ...
  if (type === 'item') {
    ...
    normalized.notes = typeof item.notes === 'string' ? item.notes.trim() : ''
  }
  return normalized
}
