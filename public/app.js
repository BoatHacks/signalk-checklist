...
function newDraftItem (type) {
  return { id: `_new_${Math.random().toString(36).slice(2, 10)}`, type, label: '', checked: false }
}
...
function Runner ({ list, connected, onToggle, onReset, onEdit, onBack }) {
...
function App () {
  ...
  const toggleItem = async (itemId, checked) => {
    ...
    try {
      await apiCall('POST', `/lists/${current.id}/items/${itemId}/check`, { checked })
    } catch (err) {
      flash('error', err.message)
    }
  }
  ...
  if (view === 'run' && current) {
    return html`<${Runner} list=${current} connected=${connected}
      onToggle=${toggleItem} onReset=${resetList}
      onEdit=${() => setView('edit')} onBack=${backToOverview} />`
  }
